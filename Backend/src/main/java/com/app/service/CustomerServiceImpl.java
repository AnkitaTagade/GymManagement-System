...
public class CustomerServiceImpl implements CustomerService {
	...
	public void deletecustomer(long id) {
	 ...
	 public List<CustomerMembershipDetailsDTO> getAllCustomers() {
	        ...
	        List<MembershipPlan> memberships = Optional.ofNullable(membershipPlanRepository.findAll()).orElse(Collections.emptyList());
	        
	        return mapCustomersToDTO(customers, memberships);
	    }
	    
	 private List<CustomerMembershipDetailsDTO> mapCustomersToDTO(List<Customer> customers, List<MembershipPlan> memberships) {
	        return customers.stream().map(customer -> {
	            MembershipPlan membership = memberships.stream()
	                .filter(m -> m.getCustomer().getCustomerId() == customer.getId())
	                .findFirst().orElse(null);
	            
	            return new CustomerMembershipDetailsDTO(
	                customer.getId(),
	                customer.getName(),
	                customer.getContact(),
	                customer.getEmail(),
	                customer.getGender(),
	                membership != null ? membership.getMembershipId() : 0,
